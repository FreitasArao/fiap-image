...
graph TB
    subgraph "Monolito Modular"
        ...
        CONSUMER[SQS Consumer<br/>CompleteMultipart]
...
S3 CompleteMultipartUpload
       ↓
   EventBridge → SNS → SQS (multipart-complete-queue)
       ↓
   API Consumer (atualiza DB, emite UPLOADED)
       ↓
   EventBridge → SNS → SQS (orchestrator-queue) → orchestrator-worker
       ↓
   orchestrator → SQS (print-queue) → print-workers (fan-out)
       ↓
   EventBridge → SNS → SQS (notification-queue) → SES
```

> **Detalhes**: Ver ADR 010 para justificativa do SQS Consumer e ADR 014 para o padrão fan-out via SNS.
